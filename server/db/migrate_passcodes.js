...
async function migratePasscodes() {
  ...
  try {
    ...
    const schemaSQL = fs.readFileSync(path.join(__dirname, 'passcodes_schema.sql'), 'utf8');
    await client.query(schemaSQL);
    console.log('Created role_passcodes table');
    
    // Check if we already have passcodes in the database
    const existingPasscodes = await client.query('SELECT COUNT(*) FROM role_passcodes');
    
    if (parseInt(existingPasscodes.rows[0].count) === 0) {
      console.log('No existing passcodes found, inserting default values from config...');
      ...
      console.log(`Found ${existingPasscodes.rows[0].count} existing passcodes, skipping insertion`);
    }
