...
const saveAssignments = async (req, res) => {
  ...
  try {
    ...
    await client.query('DELETE FROM assignment_roles WHERE service_assignment_id = $1', [serviceId]);
...
const addRole = async (req, res) => {
  ...
  try {
    ...
    if (!dateString || !roleName || !roleName.trim()) {
      return res.status(400).json({ message: 'Date string and role name are required' });
    }
...
const resetAssignments = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await client.query('DELETE FROM assignment_roles WHERE service_assignment_id = $1', [serviceId]);
      await client.query('DELETE FROM service_assignments WHERE id = $1', [serviceId]);
